...
def draw_mushroom_or_flower_rect(question_rect, red_mushroom_rect, flower_rect):
    ...
    if red_mushroom_rect in mushroom_list:
        ...
        draw_on_bg(red_mushroom_list[mushroom_count], rect = red_mushroom_rect)
...
def mushroom_rise(mushroom_rect, question_rect):
    ...
    if mushroom_rect.midbottom[1] >= question_rect.midtop[1] and \
       not mushroom_rect.bottom <= question_rect.top and mushroom_finish_rising == False:
        mushroom_rect.y -= 1
    else:
...
while True:
    ...
    if colliderect_on_bg_bottom(mario_rect, coin_brick_rect):
        ...
        # Only draw coin for the first 4 seconds of collision
        if start_time_coin:
            time_taken = pygame.time.get_ticks() - start_time_coin
            if time_taken <= 4000:
                coin_sound.play()
                draw_coin = True
                m = 1
