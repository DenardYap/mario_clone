...
while True:
        ...
        if marioEndScene == True:
            ...
            if flag_mario_drop == True:   #Drops the flag to the floor
                ...
                if mario_rect.y <336:   #Drops mario to the floor
                    mario_rect.y += 4
            
            if endgame_delay > 150: #Wait a brief before mario moves to the castle
                jump_logic = True
                
                if mario_rect.x < 527: #Mario walks to the castle
                    moving_right = True
                else:
                    moving_right = False #Mario stops walking when he reached the door

                if mario_rect.x > 520 and mario_rect.x < 530: #Mario in range of the door
                    print(win_animation)
                    win_animation += 1
                    if win_animation == 50:
                        jump = True
