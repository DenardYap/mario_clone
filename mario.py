...
def endgameSequence():
    ...
    disable_control = True  #Disable controls for animation
    moving_right = False    #Disable movement so mario sticks on the pole
    jump_logic = False      #Stop mario from sliding down the pole
    endgame_delay += 1

    if endgame_delay == 30: #Wait a brief before dropping flag and mario
        flag_drop = True
        mario_drop = True
        flag_song.play()
    
    if flag_drop == True:   #Drops the flag to the floor
        if flag_y_pos<336:
            flag_y_pos += 4

    on_ground = True

    if mario_drop == True:  #Drops mario to the floor
        if mario_y_pos<336:
            mario_y_pos += 4
